...
# here are some functions for getting the effective sample size for the matched analysis 
# since matched sets with all the same vaccination status and those with missing vac data are dropped from 
# likelihood
...
df_ve_2dose_agint_adj_comb <- regress_adj_match_ageint(df = df_combined, ocv =  ocv2) %>% 
  mutate(Duration = "12-36 months") %>% 
  mutate(ve_ci = paste0 (ve, "%", " (", ve_lb, "–", ve_ub, ")")) %>% 
  dplyr::select(Duration, category, ve_ci) %>% 
  rename_at(vars(ve_ci), ~paste(., "adj", sep = "_"))
